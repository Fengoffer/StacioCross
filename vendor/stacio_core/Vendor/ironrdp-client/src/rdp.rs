...
use ironrdp_session::{
    fast_path, ActiveStageBuilder, ActiveStageOutput, GracefulDisconnectReason, SessionResult,
};
...
pub enum RdpOutputEvent {
    Image {
        /// Tightly packed BGRA8888 bytes for one dirty desktop rectangle.
        buffer: Vec<u8>,
        x: u16,
        y: u16,
        width: NonZeroU16,
        height: NonZeroU16,
        desktop_width: NonZeroU16,
        desktop_height: NonZeroU16,
    },
...
pub fn network_characteristics_output(request: AutoDetectRequest) -> Option<RdpOutputEvent> {
    match request {
        AutoDetectRequest::NetworkCharacteristicsResult {
            ...
            average_rtt_ms,
            ..
        } => Some(RdpOutputEvent::NetworkCharacteristics {
            base_rtt_ms,
            bandwidth_kbps,
            average_rtt_ms,
        }),
...
pub enum RdpInputEvent {
    ...
    /// Apply a transport-level graphics profile. The active RDP connection is
    /// re-established because color depth and performance flags are negotiated
    /// during connection setup, not mutable Display Control properties.
    GraphicsProfile {
        color_depth: u32,
...
/// A duplicated descriptor for the real TCP transport. Keeping a duplicate
/// lets the active session sample kernel RTT after the stream has been wrapped
/// in TLS without coupling the monitor to a particular TLS implementation.
#[derive(Debug)]
struct TcpRttSampler {
    #[cfg(target_os = "macos")]
    socket: OwnedFd,
}
...
impl TcpRttSampler {
    #[cfg(target_os = "macos")]
    fn duplicate(stream: &TcpStream) -> Option<Self> {
        // SAFETY: `dup` receives a live TCP descriptor owned by `stream` and,
        // on success, returns a new independently owned descriptor.
        let descriptor = unsafe { libc::dup(stream.as_raw_fd()) };
        if descriptor < 0 {
            return None;
        }
        // SAFETY: successful `dup` transfers ownership of the new descriptor.
        let socket = unsafe { OwnedFd::from_raw_fd(descriptor) };
        Some(Self { socket })
    }

    #[cfg(not(target_os = "macos"))]
    fn duplicate(_stream: &TcpStream) -> Option<Self> {
        None
    }

    #[cfg(target_os = "macos")]
    fn average_rtt_ms(&self) -> Option<u32> {
        // SAFETY: both pointers refer to initialized, correctly sized storage;
        // getsockopt writes at most `length` bytes into `info`.
        let mut info: libc::tcp_connection_info = unsafe { core::mem::zeroed() };
        let mut length = libc::socklen_t::try_from(size_of_val(&info)).ok()?;
        let status = unsafe {
            libc::getsockopt(
        ...
        if status != 0 || usize::try_from(length).ok()? < size_of_val(&info) {
            return None;
        }
        (info.tcpi_srtt > 0).then_some(info.tcpi_srtt)
    }
...
impl RdpClient {
    ...
    pub async fn run(mut self) {
        ...
        loop {
            let (connection_result, framed, tcp_rtt_sampler) = match &self.config.transport {
                Transport::Direct => {
            ...
            .await
            {
                ...
                Ok(RdpControlFlow::ReconnectWithGraphicsProfile {
                    color_depth,
                    performance_flags,
                }) => {
                    if let Some(bitmap) = self.config.connector.bitmap.as_mut() {
                        bitmap.color_depth = color_depth;
                    }
                    self.config.connector.performance_flags = performance_flags;
                }
...
async fn connect_direct(
    ...
    input_sender: &mpsc::UnboundedSender<RdpInputEvent>,
    cliprdr_factory: CliprdrFactoryRef<'_>,
) -> ConnectorResult<(ConnectionResult, UpgradedFramed, Option<TcpRttSampler>)> {
    ...
    let tcp_rtt_sampler = TcpRttSampler::duplicate(&stream);
    ...
    let (connection_result, framed) = tls_handshake_and_finalize(framed, connector, config).await?;
    Ok((connection_result, framed, tcp_rtt_sampler))
}
...
async fn connect_gateway(
    ...
    let (connection_result, framed) = tls_handshake_and_finalize(framed, connector, config).await?;
    Ok((connection_result, framed, None))
}
...
async fn connect_rdcleanpath_transport(
    ...
    let socket = TcpStream::connect((hostname, port))
        .await
        .map_err(|e| ironrdp_connector::custom_err!("TCP connect", e))?;
    let tcp_rtt_sampler = TcpRttSampler::duplicate(&socket);
    socket
        .set_nodelay(true)
    ...
    Ok((connection_result, upgraded_framed, tcp_rtt_sampler))
}
    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + Sync + 'static,
{
    ...
    let (tls_stream, tls_cert) = ironrdp_tls::upgrade(
        initial_stream,
        config.destination.name(),
        config.ignore_certificate,
    )
    .await
...
fn bgra_image_region(
    rgba: &[u8],
    desktop_width: u16,
    desktop_height: u16,
    region: &InclusiveRectangle,
) -> Option<BgraImageRegion> {
    if desktop_width == 0
        || desktop_height == 0
        || region.left > region.right
        || region.top > region.bottom
        || region.right >= desktop_width
        || region.bottom >= desktop_height
    {
        return None;
    }

    let desktop_stride = usize::from(desktop_width).checked_mul(4)?;
    let expected_desktop_bytes = desktop_stride.checked_mul(usize::from(desktop_height))?;
    if rgba.len() != expected_desktop_bytes {
        return None;
    }
    ...
    let width_non_zero = NonZeroU16::new(width)?;
    let height_non_zero = NonZeroU16::new(height)?;
    let row_bytes = usize::from(width).checked_mul(4)?;
    let byte_count = row_bytes.checked_mul(usize::from(height))?;
    let mut buffer = Vec::with_capacity(byte_count);

    for row in region.top..=region.bottom {
        let row_start = usize::from(row)
            .checked_mul(desktop_stride)?
            .checked_add(usize::from(region.left).checked_mul(4)?)?;
        let row_end = row_start.checked_add(row_bytes)?;
        for pixel in rgba.get(row_start..row_end)?.chunks_exact(4) {
            // DecodedImage is RgbA32 while AppKit consumes little-endian BGRA.
            // The desktop surface is opaque; its alpha byte is padding in practice.
            buffer.extend_from_slice(&[pixel[2], pixel[1], pixel[0], 0xFF]);
        }
    }

    Some(BgraImageRegion {
        buffer,
        x: region.left,
        y: region.top,
        width: width_non_zero,
        height: height_non_zero,
    })
}

fn merge_dirty_region(
    pending: &mut Option<InclusiveRectangle>,
    region: InclusiveRectangle,
    desktop_width: u16,
    desktop_height: u16,
) {
    ...
    if let Some(existing) = pending.as_mut() {
        existing.left = existing.left.min(region.left);
        existing.top = existing.top.min(region.top);
        existing.right = existing.right.max(region.right);
        existing.bottom = existing.bottom.max(region.bottom);
    } else {
        *pending = Some(region);
    }
}

fn try_flush_pending_image(
    pending: &mut Option<InclusiveRectangle>,
    image: &DecodedImage,
    output_event_sender: &mpsc::Sender<RdpOutputEvent>,
) -> SessionResult<()> {
    let Some(region) = pending.as_ref() else {
        return Ok(());
    };
    let permit = match output_event_sender.try_reserve() {
        Ok(permit) => permit,
        Err(mpsc::error::TrySendError::Full(_)) => return Ok(()),
        Err(mpsc::error::TrySendError::Closed(_)) => {
            return Err(ironrdp_session::general_err!("output event receiver is closed"));
        }
    };

    let desktop_width = NonZeroU16::new(image.width())
        .ok_or_else(|| ironrdp_session::general_err!("width is zero"))?;
    let desktop_height = NonZeroU16::new(image.height())
        .ok_or_else(|| ironrdp_session::general_err!("height is zero"))?;
    let patch = bgra_image_region(image.data(), image.width(), image.height(), region)
        .ok_or_else(|| ironrdp_session::general_err!("invalid graphics update region"))?;

    permit.send(RdpOutputEvent::Image {
        buffer: patch.buffer,
        x: patch.x,
        y: patch.y,
        width: patch.width,
...
async fn active_session(
    ...
    tcp_rtt_sampler: Option<TcpRttSampler>,
    ...
    let mut graphics_flush_interval = tokio::time::interval(Duration::from_millis(16));
    graphics_flush_interval.set_missed_tick_behavior(tokio::time::MissedTickBehavior::Skip);
    let mut tcp_rtt_interval = tcp_rtt_sampler.as_ref().map(|_| {
        tokio::time::interval_at(
            tokio::time::Instant::now() + Duration::from_secs(10),
            Duration::from_secs(10),
        )
    });
    let mut pending_dirty_region: Option<InclusiveRectangle> = None;
    ...
    let disconnect_reason = 'outer: loop {
        let outputs = tokio::select! {
            ...
            input_event = input_event_receiver.recv() => {
                ...
                match input_event {
                    ...
                    RdpInputEvent::GraphicsProfile { color_depth, performance_flags } => {
                        return Ok(RdpControlFlow::ReconnectWithGraphicsProfile {
                            color_depth,
                            performance_flags,
                        });
                    }
            ...
            _ = graphics_flush_interval.tick() => Vec::new(),
            _ = async {
                match tcp_rtt_interval.as_mut() {
                    Some(interval) => interval.tick().await,
                    None => core::future::pending::<tokio::time::Instant>().await,
                }
            } => {
                if let Some(average_rtt_ms) = tcp_rtt_sampler
                    .as_ref()
                    .and_then(TcpRttSampler::average_rtt_ms)
                {
                    let _ = output_event_sender.try_send(RdpOutputEvent::NetworkCharacteristics {
                        base_rtt_ms: None,
                        bandwidth_kbps: None,
                        average_rtt_ms,
                    });
        ...
        for out in outputs {
            match out {
                ...
                ActiveStageOutput::DeactivateAll => {
                    ...
                    'activation_seq: loop {
                        ...
                        } = connection_activation.connection_activation_state()
                        {
                            ...
                            pending_dirty_region = Some(InclusiveRectangle {
                                left: 0,
                                top: 0,
                                right: desktop_size.width.saturating_sub(1),
                                bottom: desktop_size.height.saturating_sub(1),
                            });
                ...
                ActiveStageOutput::AutoDetect(request) => {
                    if let Some(event) = network_characteristics_output(request) {
                        output_event_sender
                            .send(event)
                            .await
                            .map_err(|e| ironrdp_session::custom_err!("output_event_sender", e))?;
        ...
        // Image delivery is latest-only. If Swift is busy, dirty rectangles
        // are unioned while `DecodedImage` keeps the authoritative desktop;
        // the next available slot is reconstructed from that latest image, so
        // dropping intermediate patches cannot leave permanent holes.
        try_flush_pending_image(&mut pending_dirty_region, &image, output_event_sender)?;
    };
...
mod tests {
    ...
    #[test]
    fn dirty_region_conversion_uses_bgra_order_and_tight_rows() {
        let rgba = [
        ...
        let patch = bgra_image_region(&rgba, 3, 2, &region).expect("valid patch");

        assert_eq!((patch.x, patch.y), (1, 0));
        assert_eq!((patch.width.get(), patch.height.get()), (2, 2));
        assert_eq!(
            patch.buffer,
            vec![7, 6, 5, 255, 11, 10, 9, 255, 19, 18, 17, 255, 23, 22, 21, 255]
        );
    }

    #[test]
    fn dirty_region_conversion_rejects_out_of_bounds_geometry() {
        let rgba = vec![0; 2 * 2 * 4];
        let region = InclusiveRectangle {
            left: 1,
            top: 0,
            right: 2,
        ...
        assert!(bgra_image_region(&rgba, 2, 2, &region).is_none());
    }
}
